...
names <- c("Luis", "Lina", "Camilo", "Daniel")
names
class(names)
...
# para correr una linea ctrl + enter
...
set.seed(125)
random_numbers <- sample(100:999, 10)
random_numbers
...
snp_update <- snp_data[ , -2] # Deleting the chr column
individuals = snp_data[ , 4:7] # Selecting only the columns for individuals
...
snp_data %>% filter(chr == 2 | pos == 100) # or operator
snp_data %>% filter(!chr == 2 | pos == 10) # Not and or
